...
public struct PokemonListRowView: View {
    ...
    public var body: some View {
        HStack(spacing: 16.0) {
            VStack(alignment: .leading, spacing: Constants.vStackSpacing) {
                Text(PokemonListRowView.Constants.numberPrefix + viewModel.id.formatted(withDigits: 3))
                    .font(.body)
                    .foregroundStyle(
                        Colors.primaryText.from(defaults, colorScheme: colorScheme)
                    )
                Text(viewModel.localizedNames[defaults.language] ?? Constants.emptyPokemonName)
                    .font(.headline)
                    .foregroundStyle(
    ...
    init(
        id: Int,
        localizedNames: LocalizedContentDictionary
    ) {
        _viewModel = State(
